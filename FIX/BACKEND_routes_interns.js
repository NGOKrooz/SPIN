...
const recalculateInternTimelineFromStartDate = async (intern, newStartDate, todayDate = new Date()) => {
  ...
  // FIX (issue 4): exclude any rotation still awaiting HR/operator confirmation
  // from this rebuild entirely. This function assumes a pre-planned schedule of
  // CONFIRMED rotations and walks through them by cumulative duration; a staged
  // 'awaiting_confirmation' rotation is just a proposal, not a confirmed slot.
  // Previously, pushing the start date far enough back caused this function to
  // walk PAST the current overdue rotation and land on the staged one,
  // mechanically marking the old rotation 'completed' and the staged one
  // 'active' — silently performing the exact equivalent of clicking Accept,
  // with zero confirmation. Only acceptMovement/reassignNextUnit (an explicit
  // HR action) may ever change a staged rotation's status. By excluding it
  // here, it's left completely untouched, and ensureContinuousAssignment
  // (which runs right after this function) will correctly keep finding it and
  // keep the intern in 'pending' with a growing overdue count instead.
  const rotations = allRotations.filter((rotation) => rotation.status !== 'awaiting_confirmation');
  const staleStagedRotations = allRotations.filter((rotation) => rotation.status === 'awaiting_confirmation');

  // FIX (issues 2 & 3): excluding the staged rotation from the math above isn't
  // enough on its own — if it's left in the database untouched, it still has
  // stale dates/unit from before this start-date edit, and
  // ensureContinuousAssignment (which runs right after this function) will find
  // that same stale row via its `status: 'awaiting_confirmation'` query and
  // treat it as the valid next unit. That's what was causing the "Loading next
  // unit..." stuck Accept modal and the empty "No available units for
  // reassignment" dropdown — both were reading a leftover row from an earlier
  // test cycle. Deleting it here lets ensureContinuousAssignment stage a
  // completely fresh one, with correct dates, right after this function runs.
  if (staleStagedRotations.length > 0) {
    await Rotation.deleteMany({ _id: { $in: staleStagedRotations.map((r) => r._id) } }).exec();
  }
  ...
  for (let index = 0; index < rotations.length; index += 1) {
    ...
    // FIX (issue 1): previously only extensionDays was reset to 0 here, but
    // ensureContinuousAssignment computes autoExtensionDays as
    // Math.max(existing stored value, freshly-computed overdueDays) — that
    // Math.max is meant to stop the counter ticking backwards day-to-day, but
    // it also meant a stale autoExtensionDays/manualExtensionDays from an
    // earlier test (e.g. "6 days overdue" from a previous manual date edit)
    // survived a full timeline rebuild and got shown again even when the
    // intern is now genuinely only 1 day overdue. Reset all three together.
    rotation.extensionDays = 0;
    rotation.manualExtensionDays = 0;
  ...
  // FIX (issue 1, continued): the rotation-level extension fields were reset
  // above, but the intern document has its own copies of these same fields
  // (read directly by the dashboard's status badge), which were never reset
  // here — so a stale "+6 days" from an earlier test cycle could keep showing
  // even after a fresh recompute. ensureContinuousAssignment (right after this
  // function) will set these correctly from scratch if the intern is actually
  // overdue; starting from a clean 0 here guarantees no leftover value survives
  // a timeline rebuild.
...
const syncInternRotationStates = async (internId) => {
  ...
  if (current) {
    ...
    // exist now. Extension days remain tracked as a number, just not as a status.
    intern.status = isPendingWorkflow ? 'pending' : 'active';
  } else {
    intern.status = 'completed';
  }
  await intern.save();
...
router.get('/:id/schedule', async (req, res) => {
  try {
    ...
    // staged rotation at all.
    const upcomingRotations = rawRotations.filter((rotation) => ['upcoming', 'awaiting_confirmation'].includes(rotation.status));
...
router.post('/:id/extend', async (req, res) => {
  try {
    ...
    // FIX (status model): 'extended' removed — only active, pending, completed.
    intern.status = hasPendingWorkflow ? 'pending' : 'active';
    await intern.save();
...
router.post('/:id/remove-extension', async (req, res) => {
  try {
    ...
    // FIX (status model): 'extended' removed — only active, pending, completed.
    // Preserve 'pending' if that's already the intern's state; otherwise 'active'.
    intern.status = String(intern.status || '').trim().toLowerCase() === 'pending' ? 'pending' : 'active';
    await intern.save();
