...
export default function ReassignNextModal({ confirmation, onClose, onSuccess }) {
  ...
  // FIX (issue 3): the axios response interceptor in services/api.js already
  // unwraps to response.data, so these queries' .data IS the backend payload
  // directly ({eligibleUnits: [...]} and {currentUnit, nextUnit, ...}).
  // Reading .data.data meant availableUnits was always an empty array — the
  // dropdown had nothing to select no matter how many eligible units actually
  // existed.
  const availableUnits = eligibleUnitsQuery.data?.eligibleUnits || [];
  const selectedUnit = availableUnits.find((unit) => String(unit._id || unit.id) === String(selectedUnitId)) || null;
  const nextUnitLabel = previewQuery.data?.nextUnit || 'Loading next unit...';
