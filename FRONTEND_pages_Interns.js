...
export default function Interns() {
  ...
  // FIX (status model): "Extended" is no longer a status to count — count
  // 'Pending' instead (interns overdue and awaiting HR/operator confirmation).
  const pendingCount = useMemo(() => {
    return (derivedInterns || []).filter((i) => String(i.derivedStatus || '').toLowerCase() === 'pending').length;
  }, [derivedInterns]);
                    ...
                    <div className="flex items-center space-x-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-1">
                          ...
                          {/* FIX (status model): extension days are a number tracked
                              independently of status now - show whenever > 0,
                              regardless of whether the intern is active/pending/completed. */}
