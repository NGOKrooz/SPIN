...
function generateInternRotations(intern, units, startDate) {
  ...
  // FIX (status model): 'Extended' is no longer a valid status (only active,
  // pending, completed exist now). Extension days are tracked purely as a
  // number, so check that directly instead of a status string.
  {
...
async function extendInternship(internId, extensionDays, reason, notes, unitId) {
  ...
  // FIX (status model): 'Extended' is no longer a valid status — only
  // active, pending, completed exist now. A manual extension shouldn't force
  // an intern out of 'Pending' (if HR is extending someone who's already
  // awaiting confirmation) or 'Completed'; otherwise it's 'Active'. The
  // extension day count itself is still stored below regardless.
  const currentStatusLower = String(intern.status || '').trim().toLowerCase();
  const finalStatus = (currentStatusLower === 'pending' || currentStatusLower === 'completed')
    ? intern.status
    : 'Active';
...
async function ensureInternStatusIsCorrect(internId) {
  ...
  if (hasPendingOrUpcomingAssignment) {
    ...
    // FIX (status model): 'extended' removed. Extension days remain a tracked
    // number (see refreshedIntern.extensionDays below) but no longer produce
    // their own status word — only active, pending, completed exist now.
    newStatus = existingStatus === 'pending' || isPendingConfirmation ? 'pending' : 'active';
  } else {
